...
{
$annee_debut=substr($_GET['datedebut'],6,4);
$mois_debut=substr($_GET['datedebut'],3,2);
$jour_debut=substr($_GET['datedebut'],0,2);
...
{
$annee_fin=substr($_GET['datefin'],6,4);
$mois_fin=substr($_GET['datefin'],3,2);
$jour_fin=substr($_GET['datefin'],0,2);
...
{
$hdeb=$config_prof['heureDebut'];
$hdeb = str_replace(".", "_", $hdeb);
}
else
{
$hdeb=$heure_debut_journee;
$hdeb = str_replace(".", "_", $hdeb);
}

if($config_prof['heureFin']!=0)
{
$hfin=$config_prof['heureFin'];
$hfin = str_replace(".", "_", $hfin);
}
else
{
$hfin=$heure_fin_journee;
$hfin = str_replace(".", "_", $hfin);
}
			...
			for($i=$semaine_debut;$i<=$semaine_fin;$i++)
				{
					...
					$pdf->Cell(0,5,'Semaine '.$i." de ".$annee_debut,0,0,'C');
			...
			if($j==$annee_debut)
				{
						...
						for($i=$semaine_debut;$i<=52;$i++)
						{
							...
							$pdf->Cell(0,5,'Semaine '.$i." de ".$j,0,0,'C');
