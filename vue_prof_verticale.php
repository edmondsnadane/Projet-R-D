...
if ($samedi=='1' && $dimanche=='0')
	...
	imagettftext ($im, $policedate,90,$leftwidth/2+$box_width/2, $topheight+((($hauteur-$topheight)*$nbressource/6)/2)+0*(($hauteur-$topheight)*$nbressource/6)+$box_lenght/2, $noir,$font, date("d/m",$lundi));
	...
	imagettftext ($im, $policedate,90,$leftwidth/2+$box_width/2, $topheight+((($hauteur-$topheight)*$nbressource/6)/2)+1*(($hauteur-$topheight)*$nbressource/6)+$box_lenght/2, $noir,$font, date("d/m",strtotime("+1 day",$lundi)));
	...
	imagettftext ($im, $policedate,90,$leftwidth/2+$box_width/2, $topheight+((($hauteur-$topheight)*$nbressource/6)/2)+2*(($hauteur-$topheight)*$nbressource/6)+$box_lenght/2, $noir,$font, date("d/m",strtotime("+2 day",$lundi)));
	...
	imagettftext ($im, $policedate,90,$leftwidth/2+$box_width/2, $topheight+((($hauteur-$topheight)*$nbressource/6)/2)+3*(($hauteur-$topheight)*$nbressource/6)+$box_lenght/2, $noir,$font, date("d/m",strtotime("+3 day",$lundi)));
	...
	imagettftext ($im, $policedate,90,$leftwidth/2+$box_width/2, $topheight+((($hauteur-$topheight)*$nbressource/6)/2)+4*(($hauteur-$topheight)*$nbressource/6)+$box_lenght/2, $noir,$font, date("d/m",strtotime("+4 day",$lundi)));
	...
	imagettftext ($im, $policedate,90,$leftwidth/2+$box_width/2, $topheight+((($hauteur-$topheight)*$nbressource/6)/2)+5*(($hauteur-$topheight)*$nbressource/6)+$box_lenght/2, $noir,$font, date("d/m",strtotime("+5 day",$lundi)));
...
$position+=10;
	...
	$materiels .= substr($res_materiels['nom'],0,$nb_caractere_materiel_pour_vue_prof);
			...
			imagettftext ($im, 7, 0, $leftx + ($rightx - $leftx - $box_lenght)/2 , $position-4, $noir, $font, $materiels);
				...
				//seulement pour la derniere base de données car sinon les noms sont écrits les uns sur les autres. De plus comme dans la liste déroulante il n'y a que les noms de la derniere base...
				if ($k==$nbdebdd-1)
				{
	...
	imagettftext ($im, $policegroupe,90,3*$leftwidth/4+$box_width/2, $topheight+((($hauteur-$topheight)*$nbressource/$days)/$nbressource)*($i+1)-((($hauteur-$topheight)*$nbressource/$days)/$nbressource)/2+$day*(($hauteur-$topheight)*$nbressource/$days)+$box_lenght/2, $noir,$font, $res['nom']);
...
//affectation de la couleur au type d'enseignement
	...
	// dessin des 2 cercles
		...
		imagettftext ($im, 8, 0, $leftx + ($rightx - $leftx - $box_lenght)/2 - 3, $topy + 11  , $noir, $font, $text.$horaire_debut." - ".$horaire_fin);
	...
	//pense bete   dix caracteres ont une longueur de 92px
		...
		imagettftext ($im, 9, 0, $leftx + ($rightx - $leftx - $box_lenght)/2 , ($bottomy + $topy+$epaisseur) / 2 +11 -10 * ($nbelements/2)  , $noir, $fontb, $cursename);
...
if ( $res_seance['annulee']=='1')
	...
	imagettftext ($im, $policeprof,90,3*$leftwidth/4+$box_width/2, $topheight+((($hauteur-$topheight)*$nbressource/$days)/$nbressource)*($i+$nbdegroupe+1)-((($hauteur-$topheight)*$nbressource/$days)/$nbressource)/2+$day*(($hauteur-$topheight)*$nbressource/$days)+$box_lenght/2, $noir,$font, $res['nom']);
...
$res_resas=$req_resa2->fetchAll();
	...
	// On calcule les coordonnées du rectangle :
		...
		$bottomy = round($topheight + (($hauteur - $topheight)*$nbressource) / $days * ($day + 1)-($nbressource-($i+$nbdegroupe+1))*(($hauteur - $topheight)*$nbressource / $days)/$nbressource)-10; 
...
$policesalle=6;		
	...
	imagettftext ($im, $policesalle,90,3*$leftwidth/4+$box_width/2, $topheight+((($hauteur-$topheight)*$nbressource/$days)/$nbressource)*($i+$nbdegroupe+$nbdeprof+1)-((($hauteur-$topheight)*$nbressource/$days)/$nbressource)/2+$day*(($hauteur-$topheight)*$nbressource/$days)+$box_lenght/2, $noir,$font, $res['nom']);
...
$policesalle=6;		
	...
	imagettftext ($im, $policesalle,90,3*$leftwidth/4+$box_width/2, $topheight+((($hauteur-$topheight)*$nbressource/$days)/$nbressource)*($i+$nbdegroupe+$nbdeprof+$nbdesalle+1)-((($hauteur-$topheight)*$nbressource/$days)/$nbressource)/2+$day*(($hauteur-$topheight)*$nbressource/$days)+$box_lenght/2, $noir,$font, $res['nom']);
...
imagestring($im, 1, ($largeur-94)/2, $topheight+($hauteur-$topheight)*$nbressource+60, "Généré en : ".number_format($temps_passe,3)." s ", $noir);
