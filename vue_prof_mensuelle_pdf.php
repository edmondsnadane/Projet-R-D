...
$cursename[1]=$res_seance['nom_enseignement'];
	...
	//pense bete   dix caracteres ont une longueur de 92px
		...
		imagettftext ($im, 9, 0, $leftx + ($rightx - $leftx - $box_lenght)/2 , ($bottomy + $topy) / 2 +12 -10 * ($nbelements/2)  , $noir, $fontb, $text.$cursename);

		$position=($bottomy + $topy) / 2 +12- 10 * ($nbelements/2)+10;
...
$cursename[1]=$res_seance['nom_enseignement'];
	...
	//dix caracteres ont une longueur de 92px
		...
		$position=($bottomy + $topy) / 2 +11- 10 * ($nbelements/2)+10;
...
imagestring($im, 1, ($largeur-94)/2, 5*$topheight+($hauteur-$topheight)*$nbressource+5, "Généré en : ".number_format($temps_passe,3)." s ", $noir);
