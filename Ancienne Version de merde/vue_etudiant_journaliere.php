...
$im = imagecreate ($largeur, $topheight+($hauteur-$topheight)*($nbressource)/5+75) or die ("Erreur lors de la création de l'image");
				...
				if (count($vacance_groupe)>0)
					...
					foreach ($vacance_groupe as $vacance_groupes)
						...
						if ($vacance_groupes['date']!="")
							{
								imagefilledrectangle($im, $leftwidth, $topheight, $largeur, $topheight+round((($hauteur-$topheight)*$nbressource)/5)/$nbressource, $couleur_vacances);
							}
...
// [GD] On trace les lignes, on met les jours
	...
	imagettftext ($im, $policejour,90,$leftwidth/4+$box_width/2, $topheight+((($hauteur-$topheight)*$nbressource/5)/2)+$box_lenght/2, $noir,$font,$current_day_nom);
	...
	imagettftext ($im, $policedate,90,3*$leftwidth/4+$box_width/2, $topheight+((($hauteur-$topheight)*$nbressource/5)/2)+$box_lenght/2, $noir,$font,date("d/m",mktime(0, 0, 0, date("m")  , date("d")+$jour, date("Y"))) );
...
if (count($vacance_groupe)>0)
	...
	
	{
				if ($vacance_groupes['date']!="")
					...
					imagettftext ($im, 18, 0,$leftwidth+($largeur-$leftwidth)/2  -$box_lenght/2   ,$topheight+ round((((($hauteur-$topheight)*$nbressource)/5)/$nbressource)/2)+$box_width/2  , $noir, $fontb, $text);
...
$req_seance=$dbh->prepare($sql);
	...
	// On calcule les coordonnées du rectangle :
		...
		$bottomy = round($topheight + (($hauteur - $topheight)*$nbressource) / 5 ); 
...
$req_resa_salle=$dbh->prepare($sql);
	...
	// On calcule les coordonnées du rectangle :
		...
		$bottomy = round($topheight + (($hauteur - $topheight)*$nbressource) / 5 )-10; 
...
imagestring($im, 10, ($largeur-558)/2, $topheight+($hauteur-$topheight)*$nbressource/5+13, "Légende des en-tetes :", $noir);
...
imagestring($im, 10, ($largeur-558)/2+400+40, $topheight+($hauteur-$topheight)*$nbressource/5+38, "Réservation", $noir);
...
imagestring($im, 1, ($largeur-94)/2, $topheight+($hauteur-$topheight)*$nbressource/5+60, "Généré le ".date("d/m/Y"). " a " .date("H:i:s"). " en ".number_format($temps_passe,3)."s", $noir);
